...
const { getRecetas, crearReceta, actualizarReceta, eliminarReceta, getRecetasPorUsuario, getRecetaPorId, getRecetasPorContenido, getRecetasPorTiempo } = require('../controllers/recetas');
...
router.post('/',
    [
        ...
        check('etiquetas', 'Las etiquetas de la receta son obligatorias').not().isEmpty(),
        check('status', 'El status de la receta es obligatorio').isBoolean(),
        check('fecha', 'La fecha de la receta es obligatoria').custom(isDate),
        check('tipo', 'El tipo de la receta es obligatorio').not().isEmpty(),
